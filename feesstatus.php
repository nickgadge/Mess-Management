...
if ($result->num_rows > 0) {
    echo "<table style='width:50rem' id='myTable'><tr><th>No</th><th>Name</th><th>Mobile</th><th>Date</th><th>Amount Paid</th><th>Remaining Amount</th></tr>";
    // output data of each row
    while ($row = $result->fetch_assoc()) {

        // $sqlcnt = "select count(bno) as bookcount from libiran where bno=" . $row["Bno"] . " and RDATE is null";
        // $rs = $conn->query($sqlcnt);
        // $row1 = $rs->fetch_assoc();
        $amt =$row["mmfees"];
        $remamt = 2000- $amt;
        // $amt->modify("+1 month");
        echo "<tr><td>" . $row["mmid"] . "</td><td>" . $row["mmname"] . "</td><td> " . $row["mmmob"] . "</td><td>" . $row["mmdate"] . "</td><td>" . $row["mmfees"] . "</td><td> " . $remamt . "</td></td>";
    }
    echo "</table>";
} else {
    echo "0 results";
...
<div style="padding-left:10rem;;"> <input style="padding-left:40px;size: 100px;" type="text" id="myInput" onkeyup="myFunction()" placeholder="Search for names.." title="Type in a name"><br><br><br>
    <div class="overflow-x-auto center pl-16">
        <table id="myTable" class="table-auto w-['25%']  divide-gray-200">
            <thead>
                <tr class="bg-red-100">
                    ...
                    <th class="px-6 py-3 text-left text-xl  font-medium text-gray-500 uppercase tracking-wider"> Fees Paid</th>
                 <th class="px-6 py-3 text-left text-xl  font-medium text-gray-500 uppercase tracking-wider">Remaining Fees</th>
                 <th class="px-6 py-3 text-left text-xl  font-medium text-gray-500 uppercase tracking-wider">Total Fees</th>
                </tr>
            </thead>
            <tbody class="bg-white divide-gray-200">
                ...
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        ...
                        echo "<td class='px-6 py-4 hover:bg-gray-100  whitespace-nowrap'>" . $row['mmfees'] . "</td>";
                        echo "<td class='px-6 py-4 hover:bg-gray-100  whitespace-nowrap'>" . $remamt . "</td>";
                        echo "<td class='px-6 py-4 hover:bg-gray-100  whitespace-nowrap'>" . $totamt . "</td>";
